...
test('package metadata is public-ready and clearly unofficial', () => {
  ...
  assert.equal(packageJson.bugs.url, 'https://github.com/TheStreamCode/codex-cli-launcher/issues');
...
test('package contributes launcher commands, toolbar item, and safe install settings', () => {
  ...
  assert.equal(openCliCommand.command, 'codexCliLauncher.openCli');
  assert.equal(openCliCommand.title, 'Open Codex CLI in Side Terminal');
  assert.equal(openCliCommand.category, 'Codex CLI Launcher');
  assert.deepEqual(openCliCommand.icon, {
    light: './media/launcher-mark.svg',
    dark: './media/launcher-mark.svg',
  });
  assert.equal(openSettingsCommand.command, 'codexCliLauncher.openSettings');
  assert.equal(packageJson.contributes.menus['editor/title'][0].command, 'codexCliLauncher.openCli');
  assert.equal(packageJson.contributes.menus['editor/title'][0].group, 'navigation');

  const settings = packageJson.contributes.configuration.properties;
  assert.equal(settings['codexCliLauncher.cliCommand'].default, 'codex');
...
test('legal and support documents are present and do not overclaim affiliation', () => {
  ...
  assert.match(trademarks, /This project is not affiliated with, endorsed by, sponsored by, or approved by OpenAI/i);
  ...
  assert.match(contributing, /Do not add official OpenAI or Codex logos/i);
});
